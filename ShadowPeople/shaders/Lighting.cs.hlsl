...
static const float3 SunDir      = float3(0.f, .866f, -0.5f);
static const float SunIntensity = 4.f;
...
float3 skyColor(float3 V, float3 L)
{
    ...
    if (dot(V, L) >= SunDiscCos) return SunColor * SunIntensity;

    return lerp(float3(0.7f, 0.8f, 0.9f), float3(0.3f, 0.4f, 0.95f), V.y);
}
...
void main(uint3 DTid : SV_DispatchThreadID)
{
    ...
    if (depth == 1.f) 
    {
        ...
        float3 color = skyColor(viewDir, normalize(SunDir));

        litBuffer[DTid.xy] = float4(color, 1.f); // Skip empty pixels
    }
    else
    {
        ...
        float3 color    = albedo * Ambient + SunIntensity * SunColor * (diff + spec);
